...
public class SimCharController : MonoBehaviour
{
    ...
    public MLAgent.Bones[] bones_to_apply;
    public MLAgent.Bones debug_bone;
    public Transform[] boneToTransform = new Transform[12];
    public ArticulationBody[] boneToArtBody = new ArticulationBody[12];
    public bool debug = false;
    internal Quaternion[] startingRotations = new Quaternion[12];
    private ConfigManager _config;
    // public static MocapDB db;
    public static int nbones = 12;
    ...
    private void initArticulationDrives()
    {
        for(int i = 1; i < nbones; i++)
        {
            ...
            bool musclePowerExists = _config.MusclePowers.Any(x => x.Bone == (MLAgent.Bones)i);
            Vector3 musclePower = musclePowerExists ? _config.MusclePowers.First(x => x.Bone == (MLAgent.Bones)i).PowerVector : Vector3.zero;
            if (musclePowerExists)
            {
                boneToArtBody[i].SetAllDriveStiffness(musclePower);
                // boneToArtBody[i].xDrive.stiffness = musclePower;
                // boneToArtBody[i].yDrive.stiffness = musclePower;
                // boneToArtBody[i].zDrive.stiffness = musclePower;
            }  
            else
            {    
                boneToArtBody[i].SetAllDriveStiffness(_config.boneToStiffness[i]);
                // boneToArtBody[i].xDrive.stiffness = _config.boneToStiffness[i];
                // boneToArtBody[i].yDrive.stiffness = _config.boneToStiffness[i];
                // boneToArtBody[i].zDrive.stiffness = _config.boneToStiffness[i];
            }
    ...
    public void removeBoneNearCollisions()
    {
        // Physics.IgnoreCollision(boneToCollider[(int) Bone_Shoulder].GetComponent<Collider>(), 
        //                         boneToCollider[(int) Bone_UpperArm].GetComponent<Collider>());
        // Physics.IgnoreCollision(boneToCollider[(int) Bone_UpperArm].GetComponent<Collider>(), 
        //                         boneToCollider[(int) Bone_LowerArm].GetComponent<Collider>());
        // Physics.IgnoreCollision(boneToCollider[(int) Bone_LowerArm].GetComponent<Collider>(), 
        //                         boneToCollider[(int) Bone_Palm].GetComponent<Collider>());
        // Physics.IgnoreCollision(boneToCollider[(int) Bone_Palm].GetComponent<Collider>(), 
        //                         boneToCollider[(int) Bone_Thumb1].GetComponent<Collider>());
        // Physics.IgnoreCollision(boneToCollider[(int) Bone_Palm].GetComponent<Collider>(), 
        //                         boneToCollider[(int) Bone_FingerIndex1].GetComponent<Collider>());
        // Physics.IgnoreCollision(boneToCollider[(int) Bone_Palm].GetComponent<Collider>(), 
        //                         boneToCollider[(int) Bone_FingerMiddle1].GetComponent<Collider>());
        // Physics.IgnoreCollision(boneToCollider[(int) Bone_Palm].GetComponent<Collider>(), 
        //                         boneToCollider[(int) Bone_FingerPinky1].GetComponent<Collider>());
        // Debug.Log("Done");
    }
    ...
    {
        boneToCollider = new Collider[nbones];
        for (int i = 1; i < nbones; i++)
        {
            ...
            if (i == (int) Bone_UpperArm || i == (int) Bone_LowerArm)
                ...
                boneToCollider[i] = trans.gameObject.GetComponent<CapsuleCollider>();
            else
                // boneToCollider[i] = UnityObjUtils.getChildBoxCollider(trans.gameObject).GetComponent<Collider>();
                boneToCollider[i] = trans.gameObject.GetComponent<BoxCollider>();
            if (boneToCollider[i] == null)
                Debug.Log($"Could not find collider for {(MLAgent.Bones)i }");
        }
    }
    ...
    //                                         (int) Bone_Spine1, (int) Bone_Spine, (int) Bone_Root, (int) Bone_Head};
    ...
    //     for (int i = 2; i < nbones; i++) // start at 2 because hip has no parent collider
    //     {
    ...
    public void initBoneToArtBodies()
    {
        ...
        {
            MLAgent.Bones bone = (MLAgent.Bones)i;
            ArticulationBody ab = boneToTransform[i].gameObject.GetComponent<ArticulationBody>();
            boneToArtBody[i] = ab;
        }
    }
    ...
    public static void teleportSimCharRoot(CharInfo simChar, Vector3 defaultCharPos, Vector3 simCharPosOffset)
    {
        // if (db == null)
        //     db = getDB();
        ...
        Vector3 newSimCharPos = defaultCharPos - simCharPosOffset;
    ...
    public static void teleportSimChar(CharInfo sim_char, CharInfo default_char, float verticalOffset = .15f, bool setVelocities = true)
        {
        // if (db == null)
        //     db = getDB();
        sim_char.trans.rotation = Quaternion.identity;//default_char.trans.rotation;
        // Transform kin_root = default_char.boneToTransform[(int)Bone_Shoulder];
        Transform defaultRoot = default_char.boneToTransform[(int)Bone_Shoulder];
        Transform simRoots = sim_char.boneToTransform[(int)Bone_Shoulder];
        // Adding this to root transform position will give hip transform position
        Vector3 simRootPositionOffset = sim_char.trans.position - simRoots.position;
        // we need to set: 
        // simRootPosition + simRootPositionOffset = kinHipPosition 
        // simRootPosition = kinHipPosition - simRootPositionOffset

        // We teleport the sim char a little higher to prevent it from clipping into the ground and bouncing off
        sim_char.root.TeleportRoot(defaultRoot.position + simRootPositionOffset + Vector3.up * verticalOffset, defaultRoot.rotation);
        sim_char.root.resetJointPhysics();
        if (setVelocities)
        {
            sim_char.root.velocity = Vector3.zero; //default_char.root.velocity;
        }
        for (int i = 1; i < nbones; i++)
        {
            MLAgent.Bones bone = (MLAgent.Bones)i;
            ...
            Quaternion targetLocalRot = Quaternion.identity;//default_char.boneToTransform[i].localRotation;
            ...
            setArtBodyDrivesToRotationAndReset(body, targetLocalRot, true, false);
